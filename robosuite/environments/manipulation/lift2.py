...
class Lift2(ManipulationEnv):
    """
    Lift task variant for a single robot arm (includes optional table obstacle; see :class:`Lift`).
    ...
    Args:
        ...
        placement_initializer (ObjectPositionSampler): if provided, will
            ...
            is used by default. If you pass a :class:`SequentialCompositeSampler`, the cube is attached to the first
            sub-sampler that is a :class:`UniformRandomSampler` with an empty object list.

        use_table_obstacle (bool): If True, spawn a fixed box obstacle on the table (see ``obstacle_offset``).

        obstacle_offset (2-tuple): (dx, dy) offset in meters from ``table_offset`` xy for the obstacle center.
    ...
    def _fixed_table_surface_pose(self, obj, xy_offset):
        ...
        ``z_offset=0.01`` and horizontal offsets ``xy_offset`` from ``self.table_offset``.
        """
    ...
    def _load_model(self):
        ...
        if self.placement_initializer is not None:
            if isinstance(self.placement_initializer, SequentialCompositeSampler):
                for name, sampler in self.placement_initializer.samplers.items():
                    if isinstance(sampler, UniformRandomSampler) and len(sampler.mujoco_objects) == 0:
                        self.placement_initializer.add_objects_to_sampler(name, self.cube)
                        break
            else:
            ...
            if self.table_obstacle is not None:
                ...
                composite = SequentialCompositeSampler(name="Lift2CompositeSampler")
                composite.append_sampler(obstacle_sampler)
                composite.append_sampler(cube_sampler)
                self.placement_initializer = composite
                self._builtin_obstacle_composite = True
            else:
                self.placement_initializer = cube_sampler
